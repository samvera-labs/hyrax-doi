...
# Mounted for the whole suite, not per spec type: the views build their URLs from the mount
# point, so a suite where only feature specs mount the engine cannot catch a view pointing at
# a path the host does not serve.
Rails.application.routes.send(:eval_block, proc {
